...
const Employees = () => {
  ...
  useEffect(() => {
    fetchData();
  }, []); // Remove search filters from dependencies

  const fetchData = async () => {
    try {
      setLoading(true);
      
      // Fetch all data without filters - we'll filter on frontend
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (selectedEmployee) {
        await api.put(`/employees/${selectedEmployee.employee_id}`, formData);
  ...
  const handleEdit = (employee) => {
    ...
    setFormData({
      ...
      username: '', // Don't load username for edit
  ...
  const getAvailablePositions = () => {
    if (!formData.department_id) {
      return positions; // Show all if no department selected
    }
    // Show positions that belong to the selected department OR have no department (general positions)
    return positions.filter(pos => 
      !pos.department_id || pos.department_id === Number.parseInt(formData.department_id)
    );
  };
    ...
    <div className="p-6">
      <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between">
          <div>
            <h2 className="text-2xl font-bold text-secondary-900">Quản lý Nhân viên</h2>
                  ...
                  {filteredEmployees.map((employee) => (
                    <tr key={employee.employee_id}>
                      ...
                      <td className="font-semibold">
                        {new Intl.NumberFormat('vi-VN').format(employee.salary)} đ
                      </td>
                      <td>
                        <span
                          className={`badge ${
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <select 
                      ...
                      onChange={handleInputChange}
                      required
                      disabled={!selectedEmployee && !formData.department_id}
                    >
                      <option value="">
                        {!selectedEmployee && !formData.department_id 
                    ...
                    {!selectedEmployee && !formData.department_id && (
                      <p className="text-xs text-orange-600 mt-1">
                        ⚠️ Vui lòng chọn phòng ban trước để xem chức vụ phù hợp
                      </p>
                    )}
                  </div>
                  <div>
